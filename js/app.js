const pageConfig = {
  name: document.body.dataset.shopTitle,
  location: document.body.dataset.shopLocation,
  tagline: document.body.dataset.shopTagline,
  logo: document.body.dataset.shopLogo,
...
document.querySelectorAll("[data-shop-logo]").forEach(el => {
  el.src = `../images/logos/${shop.logo}`;
  el.alt = `${shop.name} logo`;
});

document.querySelectorAll("[data-updated-date]").forEach(el => {
  el.textContent = globalRates.updatedDate;
});

document.querySelectorAll("[data-updated-time]").forEach(el => {
  el.textContent = globalRates.updatedTime;
});
...
document.title = `${shop.name} — Gold & Silver Rate`;
